...
**Live:** [https://relaxed-weasel-dokploy.cloud.nexlayer.ai](https://relaxed-weasel-dokploy.cloud.nexlayer.ai)  

**Runtime:**  · **Port:** auto-detected · **Deploy branch:** canary

---

## How this deployment works

**dokploy** is deployed on [Nexlayer](https://nexlayer.ai) — a container-native
platform where every push to `canary` triggers a fully automated build-and-deploy
...
application:
  ...
  pods:
  - name: app
    image: mirror.gcr.io/dokploy/dokploy:latest
    path: /
    servicePorts:
    - 3000
    vars:
      NODE_ENV: production
      PORT: "3000"
      DATABASE_URL: postgres://dokploy:${POSTGRES_PASSWORD}@dokploy-postgres.pod:5432/dokploy
...
Triggers on:
- **Push** to `canary` → production redeploy
...
> `<podName>.pod:<port>` syntax. CI/CD runs on push to `canary`."*
